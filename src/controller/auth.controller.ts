...
export class AuthController {
  ...
  @Redirect('/')
  async doPostSignin(
    @Body() postSigninBody: PostSigninDto,
    @Req() req: Request,
    @Res() res: Response,
  ) {
  ...
  // @Redirect('/signin')
  async doPostSignup(
    @Body() postSignupBody: PostSignupDto,
    @Res() res: Response,
  ) {
    const {
      email,
      fullname,
      password,
    }: { email: string; fullname: string; password: string } = postSignupBody;
    const signupRes: number = await this.authService.signup(
      email,
      fullname,
      password,
    );
